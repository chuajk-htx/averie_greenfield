...
from contact_lens_predictor.Prediction import predict
...
st.title("Contact Lens Detection - Live Analysis")
...
ls_all_sorted_files = get_recent_files(directory)


#if uploaded_file != None:
if ls_all_sorted_files is not None:
...
rownum = math.ceil(len(ls_files)/2) 
#st.write(rownum)
...
for row in range(0, rownum):
    ...
    for idx, (imgpath, imgcode) in enumerate(zip(ls_images, ls_files), start=1):
