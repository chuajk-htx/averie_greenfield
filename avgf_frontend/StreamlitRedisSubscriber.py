...
class StreamlitRedisImageSubscriber:
    ...
    def __init__(self, redis_servicer, st_session_state=None):
    ...
    def start_background_subscription(self, channel: str = 'upload_image'):
        ...
        if self.st_session_state:
            if 'redis_images' not in self.st_session_state:
                self.st_session_state.redis_images = []
            if 'redis_messages' not in self.st_session_state:
                self.st_session_state.redis_messages = []
            if 'subscription_status' not in self.st_session_state:
                self.st_session_state.subscription_status = "stopped"
    ...
    def _run_subscription_thread(self, channel: str):
        ...
        try:
            ...
            self.st_session_state.subscription_status = f"error: {str(e)}"
        finally:
            if self._loop:
                self._loop.close()
    ...
    async def _subscription_coroutine(self, channel: str):
        ...
        self.running = True
        self.st_session_state.subscription_status = "running"
        ...
        try:
            ...
            self.st_session_state.subscription_status = "cancelled"
        except Exception as e:
    ...
    async def _process_redis_message(self, message: Dict[str, Any]):
        ...
        try:
            ...
            # Store raw message
            self.st_session_state.redis_messages.append({
                'message': message,
                'timestamp': time.time(),
                'processed': False
            })
            ...
            if len(self.st_session_state.redis_messages) > 2:
                self.st_session_state.redis_messages = self.st_session_state.redis_messages[:2]
            
            # Process image if present
            if 'image_base64' in message and message.get("image_base64") is not None:
                image_info = await self._process_image_data(message)
                if image_info:
                    # Add to images list for display
                    self.st_session_state.redis_images.append(image_info)
                
                    # Keep only last 10 images to manage memory
                    if len(st.session_state.redis_images) > 10:
                        self.st_session_state.redis_images = self.st_session_state.redis_images[-10:]
            
            # Mark as processed
            if self.st_session_state.redis_messages:
                self.st_session_state.redis_messages[-1]['processed'] = True
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'images_count': len(self.st_session_state.get('redis_images', [])),
            'messages_count': len(self.st_session_state.get('redis_messages', [])),
            'status': self.st_session_state.get('subscription_status', 'unknown')
        }